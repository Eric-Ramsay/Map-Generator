			...
			while (i < MAPSIZE && i >= 0) {
				...
				while (j < MAPSIZE && j >= 0) {
					...
					if (map[i][j].elevation > 0) {
						...
						avg /= num;
						map[i][j].forestChance = (map[i][j].forestChance * 3 + avg) / 4.0;
					}
					if (h == 0) { j++; }
					else { j--; }
				}
	...
	for (int i = 0; i < MAPSIZE; i += 1) {
		...
		for (int j = 0; j < MAPSIZE; j += 1) {
			if (map[i][j].elevation > 10 && map[i][j].elevation < 65) {
				int chance = 990 - min(15, (map[i][j].rain/20));
				if ((rand() % 1000) > chance) {
					makeRiver(i, j);
				}
			}
...
void lakes(std::vector< std::vector<MapTile >>& map) {
	...
	auto makeLake = [&](int yC, int xC) {
		...
		if (closed.size() < MAX_SIZE && closed.size() > 15 && open.size() == 0 && !stop) {
			for (Spot c : closed) {
				map[c.y][c.x].type = 'a';
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			if (map[i][j].elevation > 5 && map[i][j].elevation < 45) {
				int chance = 990 - min(15, (map[i][j].rain / 20));
				if ((rand() % 1000) > chance) {
					makeLake(i, j);
				}
	...
	for (int m = 4; m > 0; m--) {
		for (int h = 1; h >= 0; h--) {
			...
			while (i < MAPSIZE && i >= 0) {
				int j = h * (MAPSIZE - 1);
				while (j < MAPSIZE && j >= 0) {
					if (map[i][j].elevation > 0 && !map[i][j].forest) {
						treeChance = 0;
						int adjTrees = 0;
						for (int a = -1; a < 2; a++) {
							for (int b = -1; b < 2; b++) {
								if (a != 0 || b != 0) {
									...
									if (map[safeC(i + a)][safeC(j + b)].forest) {
										treeChance += 2 * min(1, max(0, map[i][j].rain - 100) / 200.0);
										adjTrees++;
									}
								}
							}
						...
						if (adjTrees >= 3) {
							if (map[i][j].rain > 250 && map[i][j].heat > 20) {
								treeChance *= 2;
							}
							else {
								treeChance *= .1;
						...
						int chance = (treeChance) * (min(1, max(0, map[i][j].rain - 100) / 200.0) * (.5 + .5 * min(1, map[i][j].heat / 25.0)) * min(1, (100 - map[i][j].elevation)/10));
						if (rand() % 100 < chance) {
							map[i][j].forest = true;
						}
					}
...
void finalize(std::vector< std::vector<MapTile >>& map, bool forest = true) {
	MapTile t;
	bool stop = false;
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (t.elevation > 0) {
				//Cold Biomes
				if (map[i][j].heat < 25) {
					if (map[i][j].heat < 20 && map[i][j].rain < 225) {
						map[i][j].type = 'c';
					}
				...
				MapTile t = map[i][j];
				if (forest) {
					int treeChance = (t.forestChance * min(1, t.rain/500) * (.5 + .5 * min(1, t.heat/25)));
...
std::vector<std::vector<Tile>> createMap(std::vector<std::vector<MapTile>>& base) {
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (map[i][j].type == 'g') {
				map[i][j].elevation += (rand() % ((int)(20 * MAPSIZE/600.0)));
			}
			map[i][j].forestChance = rand() % 30;
			if (rand() % 100 == 0) {
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			map[i][j].rain -= map[i][j].elevation;
			map[i][j].forestChance -= map[i][j].elevation / 4;
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			gameMap[i][j] = TerrainConvert(map[i][j]);
		}
	}
	return gameMap;
}