			...
			if (i == 0) {
				...
				if (map[c.y][c.x].building == -1) {
					if (map[c.y][c.x].forest != BARE) {
						...
						case MEADOW: gIncome[WATER] += .75; gIncome[FOOD] += 1; gIncome[WORKERS] += .009; break;
						case GRASS_1: gIncome[WATER] += .6; gIncome[FOOD] += .8; gIncome[WORKERS] += .008; break;
						case GRASS_2: gIncome[WATER] += .5; gIncome[FOOD] += .6; gIncome[WORKERS] += .007; break;
						case GRASS_3: gIncome[WATER] += .3; gIncome[FOOD] += .5; gIncome[WORKERS] += .006; break;
