...
void Income(Player& p) {
	for (C c : p.land) {
		if (map[c.y][c.x].forest != NONE) {
			...
			case MEADOW: p.resources[WATER] += .4; p.resources[FOOD] += .5; break;
			case GRASS_1: p.resources[WATER] += .4; p.resources[FOOD] += .5; break;
			case GRASS_2: p.resources[WATER] += .4; p.resources[FOOD] += .4; break;
			case GRASS_3: p.resources[WATER] += .3; p.resources[FOOD] += .3; break;
			case GRASS_4: p.resources[WATER] += .2; p.resources[FOOD] += .2; break;
			case GRASS_5: p.resources[WATER] += .2; p.resources[FOOD] += .1; break;
			}
		}
	}
