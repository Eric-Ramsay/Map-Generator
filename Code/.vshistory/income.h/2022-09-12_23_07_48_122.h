			...
			if (i == 0) {
				...
				if (map[c.y][c.x].building.type == -1) {
					if (map[c.y][c.x].forest != NONE) {
						...
						case MEADOW: gIncome[WATER] += .75; gIncome[FOOD] += .75; break;
						case GRASS_1: gIncome[WATER] += .6; gIncome[FOOD] += .6; break;
						case GRASS_2: gIncome[WATER] += .5; gIncome[FOOD] += .5; break;
						case GRASS_3: gIncome[WATER] += .3; gIncome[FOOD] += .4; break;
						case GRASS_4: gIncome[WATER] += .3; gIncome[FOOD] += .3; break;
