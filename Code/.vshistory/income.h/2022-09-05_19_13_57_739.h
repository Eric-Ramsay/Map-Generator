...
void Income(Player& p) {
	for (C c : p.land) {
		if (map[c.y][c.x].forest != NONE) {
			...
			case MEADOW: p.resources[WATER] += .5; p.resources[FOOD] += .5; break;
			case GRASS_1: p.resources[WATER] += .25; p.resources[FOOD] += .25; break;
			case GRASS_2: p.resources[WATER] += .15; p.resources[FOOD] += .125; break;
			case GRASS_3: p.resources[WATER] += .1; p.resources[FOOD] += .1; break;
			case GRASS_4: p.resources[WATER] += .05; p.resources[FOOD] += .05; break;
			case GRASS_5: p.resources[WATER] += .05; p.resources[FOOD] += .025; break;
			}
		}
	}
