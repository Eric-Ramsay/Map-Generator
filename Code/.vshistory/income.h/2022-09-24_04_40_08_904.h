...
void BuildingIncome(int x, int y, float(&wIncome)[WATER + 1], float(&wMax)[WATER + 1], float (&sIncome)[WATER + 1], float (&sMax)[WATER + 1], float multiplied = 1.0) {
	...
	if (type == ROAD) {
		for (int a = -1; a < 2; a++) {
			for (int b = -1; b < 2; b++) {
				if (map[safeC(y + a)][safeC(x + b)].building.type > -1 && map[safeC(y + a)][safeC(x + b)].building.type != ROAD) {
