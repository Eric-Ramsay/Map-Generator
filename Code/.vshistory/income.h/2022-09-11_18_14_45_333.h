...
void Income(Player& p) {
	...
	float tMax[HORSES + 1] = { 100, 25, 25, -1, 50, -1, -1, 0, -1, 1000, 1000, 50, 50, 50, 50, 50, 10, 10 };
	...
	float sEff = min(1, p.res[SPECIALISTS]/sIncome[S_JOBS]);
	float wEff = 1;
	float gEff = 1;

	//Surplus of workers
	if (p.res[WORKERS] > wIncome[W_JOBS]) {
		gEff = min(1, (p.res[WORKERS] - wIncome[W_JOBS]) / gIncome[G_JOBS]);
	}

	p.resources[S_JOBS] = sIncome[S_JOBS];
	p.resources[W_JOBS] = wIncome[W_JOBS];
	p.resources[G_EFF] = gIncome[G_JOBS];
	...
	tIncome[FOOD] -= (p.resources[WORKERS] + p.resources[SPECIALISTS]) * 2;
	tIncome[WATER] -= (p.resources[WORKERS] + p.resources[SPECIALISTS]) * 2;

	float deathRate = .066;
	
	if (p.resources[FOOD] > 0 && p.resources[WATER] > 0) {
		tIncome[WORKERS] = p.resources[WORKERS] * growthRate(p.resources[WORKERS], tMax[WORKERS]);
		tIncome[SPECIALISTS] = p.resources[SPECIALISTS] * (growthRate(p.resources[SPECIALISTS], tMax[SPECIALISTS])/2);
	...
	for (int i = 0; i <= HORSES+1; i++) {
		...
		if (tMax[i] > -1 && p.resources[i] > tMax[i]) {
			p.resources[i] = tMax[i];
		}
		p.max[i] = tMax[i];
	}
}