...
void BuildingIncome(int x, int y, float(&wIncome)[WATER + 1], float(&wMax)[WATER + 1], float (&sIncome)[WATER + 1], float (&sMax)[WATER + 1], float mult = 1.0) {
	...
	float workers = buildings[map[y][x].building].costs[3];
	float specialists = 2 * buildings[map[y][x].building].costs[4];
	...
	for (int i = 0; i < 2; i++) {
		for (C c : p.land) {
			if (i == 0) {
				...
				if (map[c.y][c.x].building > NONE) {
					if (buildings[map[c.y][c.x].building].costs[3] == 0 && buildings[map[c.y][c.x].building].costs[4] == 0) {
						BuildingIncome(c.x, c.y, tIncome, tMax, sIncome, sMax);
					}
					else {
						BuildingIncome(c.x, c.y, wIncome, tMax, sIncome, sMax);
						wIncome[W_JOBS] += buildings[map[c.y][c.x].building].costs[3];
						sIncome[S_JOBS] += buildings[map[c.y][c.x].building].costs[4];
					}
				}
			}
			else {
