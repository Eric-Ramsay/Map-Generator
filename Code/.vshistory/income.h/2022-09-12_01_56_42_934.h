...
void BuildingIncome(int x, int y, float(&wIncome)[WATER + 1], float(&wMax)[WATER + 1], float (&sIncome)[WATER + 1], float (&sMax)[WATER + 1]) {
	...
	if (type == HOUSE) {
		...
		income[WATER] += 15 + 3*map[y][x].elev;
	}
	else if (type == MILL) {
		m[FOOD] += 250;
	}
	...
	float workers = buildings[map[c.y][c.x].building.type].costs[3];
	float specialists = buildings[map[c.y][c.x].building.type].costs[4];
	...
	if (workers + specialists <= 0) {
		...
		float total_pop = workers + specialists * 2;
		workers /= total_pop;
...
void Income(Player& p) {
			...
			if (i == 0) {
				...
				if (map[c.y][c.x].building.type == -1) {
					if (map[c.y][c.x].forest != NONE) {
						...
						case OCEAN: case SHALLOW: gIncome[FOOD] += .6; gIncome[GOLD] += .001; break;
	...
	tIncome[SPECIALISTS] += .00075 * p.res[WORKERS];
	tIncome[WORKERS] -= .00075 * p.res[WORKERS];
