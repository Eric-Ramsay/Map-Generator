...
void Income(Player& p) {
	...
	income[WORKERS] +=  p.res[WORKERS] * -deathRate;
	income[SPECIALISTS] += p.res[SPECIALISTS] * -deathRate/2.0;

	income[GOLD] += .0075 * p.res[WORKERS];
	income[GOLD] += .03 * p.res[SPECIALISTS];

	income[SPECIALISTS] += .001 * p.res[WORKERS];
	income[WORKERS] -= .001 * p.res[WORKERS];

	p.power += .1;

	for (int i = 0; i <= HORSES+1; i++) {
		p.res[i] = max(0, p.res[i] + income[i]);
