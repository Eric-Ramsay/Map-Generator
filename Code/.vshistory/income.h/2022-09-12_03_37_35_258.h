...
void Income(Player& p) {
		...
		for (C c : p.land) {
			if (i == 0) {
				...
				if (map[c.y][c.x].building.type != -1) {
					BuildingIncome(c.x, c.y, wIncome, wMax, sIncome, sMax);
	...
	if (p.res[FOOD] > 0 && p.res[WATER] > 0) {
		...
		tIncome[SPECIALISTS] = p.res[SPECIALISTS] * (growthRate(p.res[SPECIALISTS], tMax[SPECIALISTS])/4);
	...
	tIncome[SPECIALISTS] += p.res[SPECIALISTS] * -deathRate;
