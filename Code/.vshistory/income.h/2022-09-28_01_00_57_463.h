		...
		for (C c : p.land) {
			if (i == 0) {
				if (map[c.y][c.x].owner == p.turn) {
					//First make sure that the tile's income hasn't already been somehow accounted for.
					bool found = false;
					...
					if (!found) {
						...
						// Tile has already been processed, don't count it twice.
						break;
					}
				}
				if (map[c.y][c.x].building > -1) {
					// If the building requires no workers, worker efficiency doesn't matter
					if (UI.buildings[map[c.y][c.x].building].costs[3] == 0 && UI.buildings[map[c.y][c.x].building].costs[4] == 0) {
						...
						// Otherwise, keep track of worker numbers and keep specialist/worker income seperate
						BuildingIncome(c.x, c.y, wIncome, tMax, sIncome, sMax);
						tIncome[W_JOBS] += UI.buildings[map[c.y][c.x].building].costs[3];
						tIncome[S_JOBS] += UI.buildings[map[c.y][c.x].building].costs[4];
					}
				}
			}
			else {
				//No building on the tile, deal with the gathering income from it (ie forest, fishing, etc)
				if (map[c.y][c.x].building == -1) {
