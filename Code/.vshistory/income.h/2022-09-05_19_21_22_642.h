...
void Income(Player& p) {
	for (C c : p.land) {
		if (map[c.y][c.x].forest != NONE) {
			...
			case MEADOW: p.resources[WATER] += .6; p.resources[FOOD] += .6; break;
			case GRASS_1: p.resources[WATER] += .5; p.resources[FOOD] += .5; break;
			...
			case GRASS_5: p.resources[WATER] += .1; p.resources[FOOD] += .1; break;
			}
		}
	}
