...
void Income(Player& p) {
	...
	if (p.resources[FOOD] > 0 && p.resources[WATER] > 0) {
		tIncome[WORKERS] = p.resources[WORKERS] * growthRate(p.resources[WORKERS], p.max[WORKERS]);
		tIncome[SPECIALISTS] = p.resources[SPECIALISTS] * (growthRate(p.resources[SPECIALISTS], p.max[SPECIALISTS])/2);
