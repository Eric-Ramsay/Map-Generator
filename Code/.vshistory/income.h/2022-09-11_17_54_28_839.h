...
void Income(Player& p) {
	...
	float wM[HORSES + 1] = { 100, 25, 25, -1, 50, -1, -1, 0, -1, 1000, 1000, 50, 50, 50, 50, 50, 10, 10 };
	float sM[HORSES + 1] = { 100, 25, 25, -1, 50, -1, -1, 0, -1, 1000, 1000, 50, 50, 50, 50, 50, 10, 10 };
			...
			if (i == 0) {
				...
				if (map[c.y][c.x].building.type != -1) {
					if (buildings[map[c.y][c.x].building.type].specialists > 0) {
						BuildingIncome(c.x, c.y, sIncome, m);
					}
