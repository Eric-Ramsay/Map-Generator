...
void Income(Player& p) {
	...
	for (int i = 0; i < 2; i++) {
			...
			if (i == 0) {
				...
				if (map[c.y][c.x].building.type == -1) {
					if (map[c.y][c.x].forest != NONE) {
						...
						case LAKE: case RIVER: gIncome[WATER] += 2; gIncome[FOOD] += 5; break;
						case OCEAN: case SHALLOW: gIncome[FOOD] += .75; gIncome[GOLD] += .0075; break;
						case MEADOW: gIncome[WATER] += .5; gIncome[FOOD] += .7; break;
						case GRASS_1: gIncome[WATER] += .4; gIncome[FOOD] += .6; break;
						case GRASS_2: gIncome[WATER] += .4; gIncome[FOOD] += .5; break;
						case GRASS_3: gIncome[WATER] += .3; gIncome[FOOD] += .4; break;
						case GRASS_4: gIncome[WATER] += .2; gIncome[FOOD] += .3; break;
						case GRASS_5: gIncome[WATER] += .2; gIncome[FOOD] += .2; break;
		...
		if (i == 1) {
			gIncome[G_JOBS] = (gIncome[STONE] + gIncome[GOLD] + gIncome[WOOD]) * 3 + (gIncome[FOOD] + gIncome[WATER]) * .25;
		}
	}
	...
	tIncome[SPECIALISTS] += .005 * p.res[WORKERS];
	tIncome[WORKERS] -= .005 * p.res[WORKERS];
