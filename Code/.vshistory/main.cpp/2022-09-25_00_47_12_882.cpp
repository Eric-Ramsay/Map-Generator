		...
		if (UI.inGame) {
			...
			if (UI.inGame && sf::Mouse::getPosition().x < (UI.W - 320)) {
				if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
					...
					else if (UI.building > 0 && map[UI.tH.y][UI.tH.x].owner == p.turn && map[UI.tH.y][UI.tH.x].building == -1) {
						if (map[UI.tH.y][UI.tH.x].elev > DEEP && map[UI.tH.y][UI.tH.x].forest == BARE) {
							if (p.res[WOOD] > UI.buildings[UI.building].costs[0] && p.res[STONE] > UI.buildings[UI.building].costs[1] && p.res[GOLD] > UI.buildings[UI.building].costs[2]) {
								map[UI.tH.y][UI.tH.x].building = (BUILDING)UI.building;
								p.res[WOOD] -= UI.buildings[UI.building].costs[0];
								p.res[STONE] -= UI.buildings[UI.building].costs[1];
								p.res[GOLD] -= UI.buildings[UI.building].costs[2];
		...
		while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed) {
				...
				if (event.mouseButton.button == sf::Mouse::Left) {
					...
					if (map[UI.tH.y][UI.tH.x].owner == p.turn) {
						if (map[UI.tH.y][UI.tH.x].building > NONE) {
							map[UI.tH.y][UI.tH.x].building = NONE;
						}
						else {
							map[UI.tH.y][UI.tH.x].owner = 0;
							p.res[POWER]++;
