...
int main() {
	UI.w = 1920;
	UI.h = 1080;
	window = sf::RenderWindow(sf::VideoMode(UI.W, UI.H), "TG2 DE", sf::Style::Fullscreen);
	...
	while (window.isOpen()) {
		...
		if (inGame) {
			...
			UI.mX /= ((W - 320) / texture.getSize().x);
			UI.mY /= (H / texture.getSize().y);

			tH.x = safeC(xView + floor((UI.mX - (xShift + tX / 2.0)) / 16.0));
			tH.y = safeC(yView + floor((UI.mY - (yShift + tY / 2.0)) / 16.0));

			if (inGame && sf::Mouse::getPosition().x < (W - 320)) {
				if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
					...
					else if (building > 0 && map[tH.y][tH.x].owner == p.turn && map[tH.y][tH.x].building == NONE) {
						if (map[tH.y][tH.x].elev > DEEP && map[tH.y][tH.x].forest == NONE) {
