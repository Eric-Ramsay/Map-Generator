	...
	while (window.isOpen()) {
		...
		if (inGame) {
			...
			if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
				...
				else if (building > -1 && map[tH.y][tH.x].owner == p.turn && map[tH.y][tH.x].building.type == -1) {
					if (p.resources[WOOD] > buildings[building].costs[0] && p.resources[STONE] > buildings[building].costs[1] && p.resources[GOLD] > buildings[building].costs[2])
					map[tH.y][tH.x].building.type = building;
					p.resources[WOOD] -= buildings[building].costs[0];
					p.resources[STONE] -= buildings[building].costs[1];
					p.resources[GOLD] -= buildings[building].costs[2];
