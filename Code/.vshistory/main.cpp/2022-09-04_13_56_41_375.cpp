	...
	while (window.isOpen()) {
		...
		if (inGame) {
			...
			if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
				bool canClaim = p.land.size() == 0 && map[tH.y][tH.x].elev > DEEP && map[tH.y][tH.x].elev <= HILL;
				if (!canClaim) {
					for (int i = -1; i < 2; i++) {
						for (int j = -1; j < 2; j++) {
							if (map[safeC(tH.y + i)][safeC(tH.x + j)].elev >= FLAT) {
								if (map[safeC(tH.y + i)][safeC(tH.x + j)].owner == p.s) {
									canClaim = true;
									break;
								}
							}
		...
		drawSprite(528, 16, 16, 16, 3, 3, 10, 28);

		for (int i = 0; i < resources.size(); i++) {
			Print(resources[i] + ": " + trimNum(p.resources[i]) + " / " + trimNum(p.max[i]), 10, 10 + 30 * i, 1, false);
		}
