...
int main() {
	...
	while (window.isOpen()) {
		...
		if (inGame) {
			...
			tH.x = safeC(xView + ((mX - 8) - tX / 2.0) / 16);
			tH.y = safeC(yView + ((mY - 8) - tY / 2.0) / 16);
		...
		texture.create(960.0 / max(1, zoom), 540.0 / max(1, zoom));
		texture.clear(sf::Color::Red);
		DrawMap();

		int dX = 16;
		int dY = 0;
		drawSprite(208, 496, 16, 16, dX, dY);
		drawPosition(mX, mY);

		texture.display();
		sf::Sprite sprite;
		sprite.setTexture(texture.getTexture());
		sprite.setScale(960.0 /texture.getSize().x, 540.0 /texture.getSize().y);
		window.draw(sprite);
