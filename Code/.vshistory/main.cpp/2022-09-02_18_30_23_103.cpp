...
int main() {
	...
	while (window.isOpen()) {
		...
		if (inGame) {
			...
			float x_tiles = ceil(tX / 16.0) + 4;
			float x_tile_start_coord = tX / 2.0 - x_tiles * 8;

			mX += x_tile_start_coord;

			mX -= ((ceil(texture.getSize().x / 16.0) + 4) - (texture.getSize().x)) / 2.0;
			mY -= ((ceil(texture.getSize().y / 16.0) + 4) - (texture.getSize().y)) / 2.0;

			tH.x = safeC(xView + (mX - texture.getSize().x/2.0) / 16);
			tH.y = safeC(yView + (texture.getSize().y/2.0 - mY) / 16);


			//tH.x = safeC(xView + (sf::Mouse::getPosition().x - window.getSize().x / 2.0) / (16.0 * zoom));
			//tH.y = safeC(yView - (sf::Mouse::getPosition().y - window.getSize().y / 2.0) / (16.0 * zoom));
