...
int main() {
	...
	while (window.isOpen()) {
		...
		sf::Event event;
		if (inGame) {
			tH.x = safeC(xView + (((sf::Mouse::getPosition().x - xShift) - window.getSize().x / 2.0) / (16.0 * (float)zoom)));
			tH.y = safeC(yView + (((-sf::Mouse::getPosition().y + yShift) + window.getSize().y / 2.0) / (16.0 * (float)zoom)));
