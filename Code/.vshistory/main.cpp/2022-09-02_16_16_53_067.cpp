...
int main() {
	...
	while (window.isOpen()) {
		...
		sf::Event event;
		if (inGame) {
			tH.x = safeC(xView + floor((sf::Mouse::getPosition().x + xShift - texture.getSize().x / 2.0) / (float)zoom));
			tH.y = safeC(yView + floor((sf::Mouse::getPosition().y + yShift - texture.getSize().y / 2.0) / (float)zoom));
