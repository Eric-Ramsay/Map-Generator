...
int main() {
	...
	while (window.isOpen()) {
		...
		if (inGame) {
			...
			int x_max = texture.getSize().x; //(ceil(texture.getSize().x / 16.0) + 4) * 16;
			int y_max = texture.getSize().y; //(ceil(texture.getSize().y / 16.0) + 4) * 16;

			tH.x = safeC(xView + (mX - x_max/2.0) / 16);
