...
int main() {
	...
	while (window.isOpen()) {
		...
		sf::Event event;
		if (inGame) {
			tH.x = 1 + safeC(xView + floor(((sf::Mouse::getPosition().x - xShift) - window.getSize().x / 2.0) / (16 * (float)zoom)));
			tH.y = 1 + safeC(yView + floor(((-sf::Mouse::getPosition().y + yShift) + window.getSize().y / 2.0) / (16 * (float)zoom)));
