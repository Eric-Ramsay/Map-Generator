			...
			if (UI.inGame && sf::Mouse::getPosition().x < (UI.W - 320)) {
				if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
					if (UI.click == CLAIM && map[UI.tH.y][UI.tH.x].owner == 0) {
						...
						if (map[UI.tH.y][UI.tH.x].res[WORKERS] >= 2) {
							map[UI.tH.y][UI.tH.x].res.wood += .5;
							map[UI.tH.y][UI.tH.x].res[WORKERS] -= 2;
						}
					}
					else if (UI.click == CLEAR && map[UI.tH.y][UI.tH.x].owner == p.turn && map[UI.tH.y][UI.tH.x].building == -1 && map[UI.tH.y][UI.tH.x].forest == BARE) {
