...
int main() {
	...
	while (window.isOpen()) {
		...
		sf::Event event;
		if (inGame) {
			tH.x = safeC(1 + xView + floor(((sf::Mouse::getPosition().x - xShift) - window.getSize().x / 2.0) / (16 * (float)zoom)));
			tH.y = safeC(1 + yView + floor(((-sf::Mouse::getPosition().y + yShift) + window.getSize().y / 2.0) / (16 * (float)zoom)));
