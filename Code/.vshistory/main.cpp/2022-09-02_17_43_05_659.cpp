...
int main() {
	...
	while (window.isOpen()) {
		...
		sf::Event event;
		if (inGame) {
			tH.x = safeC(xView + floor((sf::Mouse::getPosition().x - window.getSize().x / 2.0) / (16 * (float)zoom)));
			tH.y = safeC(yView + floor((-sf::Mouse::getPosition().y + window.getSize().y / 2.0) / (16 * (float)zoom)));
