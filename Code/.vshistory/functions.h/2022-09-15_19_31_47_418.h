...
Tile Terrain_Convert(MapTile t) {
	...
	if (t.forest) {
		...
		else if (t.type == '5' || t.type == '4' || t.type == 'p' || t.type == 'c' || t.type == '3') {
			if (t.elevation > 30) {
				til.forest = GLADE;
				int ape = rand() % 9;
				if (ape == 0) {
...
std::string trimNum(float x, int throttle = -1, bool prettify = false) {
	char append = '!';
	int excess = 0;
	float num;
	if (x < 1) { //2 decimal precision
		
	}
	else if (x < 100) { //1 decimal allowed
