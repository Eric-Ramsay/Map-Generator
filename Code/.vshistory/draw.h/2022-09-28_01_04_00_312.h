...
void DrawRightBar() {
	...
	if (map[UI.tH.y][UI.tH.x].building == -1) {
		...
		tIncome[W_JOBS] = UI.buildings[map[tH.y][tH.x].building].costs[3];
		tIncome[S_JOBS] = UI.buildings[map[tH.y][tH.x].building].costs[4];
			...
			for (int i = 0; i < WATER + 1; i++) {
				if (a == 0) {
					if (tIncome[i] > 0.01 || tIncome[i] < 0.01) {
						Print(words[i] + ": " + trimNum(tIncome[i]), 10, dY);
						dY += 15;
					}
				}
				else {
					if (tMax[i] > 0.01 || tMax[i] < 0.01) {
						Print("Max " + words[i] + ": " + trimNum(tIncome[i]), 10, dY);
					}
				}
			}
		}
