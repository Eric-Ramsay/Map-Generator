...
void drawSprite(int sX, int sY, int UI.W, int h, float dX, float dY, int scale = 1, sf::Color color = sf::Color(255, 255, 255), bool windraw = false) {
...
void drawSprite(int sX, int sY, int UI.W, int h, float dX, float dY, float x_scale, float y_scale, sf::Color color = sf::Color(255, 255, 255), bool windraw = false) {
	static bool init = false;
	static sf::Texture sprites;
	if (!init) {
		sprites = createTexture("./Sprites/sprites.png");
...
void Print(std::string text, float x = -1, float y = -1, float scale = 1.0f, bool center = true, int width = 1920 / 2) {
	...
	for (char& c : text) {
		...
		if (c == '*') {
			...
			if (prnt) {
				//drawSprite(int sX, int sY, int UI.W, int h, int dX, int dY, int scale = zoom, sf::Color color = sf::Color(255, 255, 255))
...
void DrawRightBar() {
	...
	drawSprite(528, 16, 16, 16, UI.W - 320, 0, 20, H / 16.0);
	//Right Bar Stuff -----
