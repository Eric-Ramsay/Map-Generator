...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (UI.zoom >= 1) {
		...
		//Draw the base tile
		drawSprite(sX + rX * 16, sY + 16 + rY * 16	, 16, 16, dX, dY, shade);
		TERRAIN adjType;
		for (int a = -1; a < 2; a++) {
			for (int b = -1; b < 2; b++) {
				if ((a == 0) != (b == 0)) {
					...
					if (typePrecedent(adjType) > typePrecedent(map[y][x].type)) {
						...
						if (side > 0) {
							if (b == -1) {
								drawSprite(sX + 14, sY, 2, 16, dX, dY, shade);
							}
							else {
								drawSprite(sX, sY, 2, 16, dX + 14, dY, shade);
							}
						}
						else {
							if (a == -1) {
								drawSprite(sX + 16, sY + 14, 16, 2, dX, dY, shade);
							}
							else {
								drawSprite(sX + 16, sY, 16, 2, dX, dY + 14, shade);
							}
						}
					}
				}
		...
		if (UI.tH.y == y && UI.tH.x == x) {
			...
			else if (UI.click == CLEAR) {
				drawSprite(400, 560, 16, 16, dX, dY + offset);
			}
			else if (UI.click == PLANT && map[y][x].forest == BARE) {
				drawSprite(416, 560, 16, 16, dX, dY + offset);
			}
		}
	}
	else {
		...
		drawSprite(sX, sY, 2, 2, dX, dY, 1, color, shade);
	}
}
