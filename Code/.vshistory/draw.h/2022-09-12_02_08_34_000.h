...
void DrawUI() {
	...
	//WORKERS, SPECIALISTS, G_JOBS, W_JOBS, S_JOBS, WELLBEING, WOOD, STONE, GOLD, FOOD, WATER, LUXURIES, COPPER, TIN, BRONZE, IRON, CATTLE, HORSES
	...
	for (int i = 0; i < resources.size(); i++) {
		std::string prnt = resources[i] + ": " + trimNum(p.res[i]);
		if (p.max[i] > -1) {
	...
	Print("*BLUE*Workers: *BLUE*" + to_str(buildings[selected].costs[0]), rX + rW / 4, rY + rH - 260, 2, true);
	Print("*ORANGE*Specialists: *ORANGE*" + to_str(buildings[selected].costs[1]), rX + 3 * rW / 4, rY + rH - 260, 2, true);
