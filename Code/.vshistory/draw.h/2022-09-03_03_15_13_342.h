...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (zoom >= 1) {
		...
		if (map[y][x].elev > LOW) {
			color.a += map[y][x].elev * 25;
		}
		drawSprite(sX, sY, 4, 4, dX, dY, 1, color);
	}
}
...
void DrawMap() {
	if (zoom >= 1) {
		...
		int x_max = ceil(texture.getSize().x / 4.0) + 4;
		int y_max = ceil(texture.getSize().y / 4.0) + 4;
		...
		for (int i = -ceil(y_max / 2.0); i <= y_max / 2; i++) {
			for (int j = -ceil(x_max / 2.0); j <= x_max / 2; j++) {
				int y = safeC(i + yView);
				int x = safeC(j + xView);
				DrawTile(map, x, y, xShift + j * 4 + texture.getSize().x / 2.0, yShift + i * 4 + texture.getSize().y / 2.0);
			}
		}
	}
}