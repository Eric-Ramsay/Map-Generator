...
std::string fNum(float num) {
	return to_str((int)(100 * num) / 100.0);
}
...
void DrawResources() {
	...
	for (int i = 0; i < 2; i++) {
		...
		Print(to_str((int)p.res[WORKERS + i]), dX + 26.5 * scale, dY + 7.5 * scale, scale);
		...
		Print(shade + to_str(p.income[WORKERS + i]) + shade, dX + 26.5 * scale, dY + 17.5 * scale, scale);
	...
	for (int i = 0; i < 4; i++) {
		...
		Print(to_str((int)p.res[POWER + i]), dX + 26.5 * scale, dY + 7.5 * scale, scale);
		...
		Print(shade + to_str(p.income[POWER + i]) + shade, dX + 26.5 * scale, dY + 17.5 * scale, scale);
	...
	drawSprite(sX, sY, sW, sH, dX, dY, scale);
	for (int i = 0; i < 3; i++) {
		Print(to_str((int)p.res[G_JOBS + i]), dX + 26.5 * scale, dY + (float)(8.5 + i * 12) * scale, scale);
	}
	...
	for (int i = 0; i < 2; i++) {
		...
		Print(to_str((int)p.res[FOOD + i]), dX + 26.5 * scale, dY + 7.5 * scale, scale);
		...
		Print(shade + to_str(p.income[FOOD + i]) + shade, dX + 26.5 * scale, dY + 17.5 * scale, scale);
