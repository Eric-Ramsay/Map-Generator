...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (zoom >= 1) {
		...
		if (map[y][x].forest != NONE) {
			...
			drawSprite(sX, sY, 16, 32, dX, dY + offset - (16));
		}
...
void DrawUI() {
	...
	drawSprite(528, 16, 16, 16, 0, 0, 20, H / 16.0);
	...
	for (int i = 0; i < buildings.size(); i++) {
		int scale = 3;
		if (i != building) {
			drawSprite(112, 160, 16, 32, rX + 10 + (10 + 24 * scale) * (i % 4), rY + 10 + (10 + 40 * scale) * (i / 4), scale);
		}
		else {
			drawSprite(128, 160, 16, 32, rX + 10 + (10 + 24 * scale) * (i % 4), rY + 10 + (10 + 40 * scale) * (i / 4), scale);
		}
		drawSprite(sX + 16 * i, sY, 16, 32, rX + 10 + (10+24*scale) * (i%4), rY + 10 + (10 + 40*scale) * (i/4), scale);
	}

	Print(buildings[selected].name, rX + rW / 2, rY + rH - 250, 2, true);
	//Draw Cost of Current Building:
	Print("*BROWN*Wood: *BROWN*" + to_str(buildings[selected].costs[2]), rX + rW / 4, rY + rH - 190, 2, true);
	Print("*GREY*Stone: *GREY*" + to_str(buildings[selected].costs[3]), rX + rW / 4, rY + rH - 120, 2, true);
	Print("*GOLD*Gold: *GOLD*" + to_str(buildings[selected].costs[4]), rX + 3 * rW / 4, rY + rH - 190, 2, true);
