...
void DrawBuilding(int x, int y, int dX, int dY, sf::Color color) {
	int sY = 80;
	int sX = 16 * map[y][x].building.type;
	if (map[y][x].building.type == ROAD) {
		sY = 112;
		//Check if adjacent tiles are also roads
		bool u = map[safeC(y - 1)][x].building.type == ROAD; 
		bool d = map[safeC(y + 1)][x].building.type == ROAD;
		bool l = map[y][safeC(x - 1)].building.type == ROAD;
		bool r = map[y][safeC(x + 1)].building.type == ROAD;
		sX = 16 * ((u * 8) + (d * 4) + (l * 2) + r);
	}
	else {
		drawSprite(sX, sY, 16, 32, dX, dY - (16), color);
	}
}
...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (zoom >= 1) {
		...
		if (tH.y == y && tH.x == x) {
			...
			if (building > -1) {
				DrawBuilding(x, y, dX, dY, sf::Color(255, 255, 255, 150));
			}
		}
	}
	else {
