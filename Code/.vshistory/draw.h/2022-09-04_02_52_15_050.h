			...
			for (int b = -1; b < 2; b++) {
				if ((a == 0) != (b == 0)) {
					...
					if (typePrecedent(adjType) > typePrecedent(map[y][x].type)) {
						...
						if (side > 0) {
							if (b == -1) {
								drawSprite(sX + 14, sY, 2, 16, dX - 15, dY);
							}
							else {
								drawSprite(sX, sY, 2, 16, dX + 15, dY);
							}
						}
						else {
							if (a == -1) {
								drawSprite(sX + 16, sY + 14, 16, 2, dX, dY - 15);
							}
							else {
								drawSprite(sX + 16, sY, 16, 2, dX, dY + 15);
							}
						}
					}
				}
