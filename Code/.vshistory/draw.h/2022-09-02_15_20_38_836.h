...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	//Draw the base tile
	drawSprite(sX, sY + 17, 16, 16, dX, dY);
	return;
	TERRAIN adjType;
	for (int a = -1; a < 2; a++) {
		for (int b = -1; b < 2; b++) {
			if ((a == 0) != (b == 0)) {
				...
				if (typePrecedent(adjType) > typePrecedent(map[y][x].type)) {
					...
					if (side > 0) {
						if (b == -1) {
							drawSprite(sX + 14, sY, 2, 16, dX - 8, dY);
						}
						else {
							drawSprite(sX, sY, 2, 16, dX + 7, dY + 0);
						}
					}
					else {
						if (a == -1) {
							drawSprite(sX + 16, sY + 14, 16, 2, dX, dY - 8);
						}
						else {
...
void DrawMap() {
	int x_max = ceil(texture.getSize().x / 16.0) + 2;
	int y_max = ceil(texture.getSize().y / 16.0) + 2;

	for (int i = -(x_max/2); i <= y_max/2; i++) {
		for (int j = -(y_max/2); j <= x_max/2; j++) {
			int y = safeC(i + yView);
			int x = safeC(j + xView);
			DrawTile(map, x, y, xShift + j * 16, yShift + i * 16 - texture.getSize().y/2.0);
		}
	}
}

/*void DrawMap() {
	int x_max = ceil((1920.0 / zoom)/16.0) + 3;
	int y_max = ceil((1080.0 / zoom) / 16.0) + 3;
	for (int i = -y_max/2.0; i < y_max/2.0; i++) {
		for (int j = -x_max/2.0; j < x_max/2.0; j++) {
			int y = safeC(i + yView);
			int x = safeC(j + xView);
			DrawTile(map, x, y, xShift + j * 16 + 1920/2.0, yShift + i * 16 + 1080/2.0);
		}
	}
}*/