...
void drawSprite(int sX, int sY, int w, int h, float dX, float dY, int scale = 1, sf::Color color = sf::Color(255, 255, 255)) {
	...
	if (UI.WINDOW_DRAW) {
		window.draw(sprite, sf::RenderStates(sf::BlendMultiply));
	}
	else {
		texture.draw(sprite, sf::RenderStates(sf::BlendMultiply));
	}
}
...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (season == AUTUMN) {
		shade = sf::Color(195, 175, 125, 200);
		std::cout << (int)shade.r << (int)shade.g << (int)shade.b << (int)shade.a << std::endl;
	}
	if (UI.zoom >= 1) {
