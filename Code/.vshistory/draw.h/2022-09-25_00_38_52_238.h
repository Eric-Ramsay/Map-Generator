...
void DrawBuilding(int x, int y, int dX, int dY) {
	...
	if (map[y][x].building == ROAD) {
		...
		sX = 16 * ((u * 8) + (d * 4) + (l * 2) + r);
		drawSprite(sX, sY, 16, 32, dX, dY - (8));
	}
	else {
...
void DrawBuilding(BUILDING type, int dX, int dY, sf::Color color = sf::Color(255, 255, 255)) {
	...
	if (type == ROAD) {
		sY = 112;
		sX = 0;
		drawSprite(sX, sY, 16, 32, dX, dY - (8), 1, color);
	}
	else {
...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (UI.zoom >= 1) {
		...
		if (map[y][x].elev == HILL || map[y][x].elev == MOUNTAIN || map[y][x].forest != NONE || map[y][x].building > NONE) {
			useRand = 0;
		}
		...
		if (UI.tH.y == y && UI.tH.x == x) {
			...
			if (UI.building) {
				DrawBuilding(UI.building_selected, dX, dY, sf::Color(255, 255, 255, 150));
			}
		}
	}
	else {
...
void DrawRightBar() {
		...
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
			if (UI.mX >= x_pos && UI.mY >= y_pos) {
				if (UI.mX <= x_pos + 16 * scale && UI.mY <= y_pos + 32 * scale) {
					UI.building_selected = (BUILDING)(i+1);
					UI.building = false;
				}
			}
		}
		if (i != UI.building_selected - 1) {
	...
	Print("*BLUE*Workers: *BLUE*" + to_str(UI.buildings[selected].costs[3]), rX + rW / 4, rY + rH - 260, 2, true);
	Print("*ORANGE*Spec.: *ORANGE*" + to_str(UI.buildings[selected].costs[4]), rX + 3 * rW / 4, rY + rH - 260, 2, true);
	Print("*BROWN*Wood: *BROWN*" + to_str(UI.buildings[selected].costs[0]), rX + rW / 4, rY + rH - 190, 2, true);
	Print("*GREY*Stone: *GREY*" + to_str(UI.buildings[selected].costs[1]), rX + rW / 4, rY + rH - 120, 2, true);
	Print("*GOLD*Gold: *GOLD*" + to_str(UI.buildings[selected].costs[2]), rX + 3 * rW / 4, rY + rH - 190, 2, true);
	Print("*GREEN*Time: *GREEN*" + to_str(UI.buildings[selected].time), rX + 3 * rW / 4, rY + rH - 120, 2, true);
