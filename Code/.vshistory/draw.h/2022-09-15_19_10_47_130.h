...
void DrawResources() {
	...
	for (int i = 0; i < 2; i++) {
		...
		drawSprite((WORKERS + 2 * i) * 16, 576, 11, 11, dX + 2 * scale, dY + 2 * scale, scale);
		dY += (sH + 1) * scale;
	}
	...
	for (int i = 0; i < 4; i++) {
		...
		Print(to_str(p.income[POWER + i]), dX + 26.5 * scale, dY + 17.5 * scale, scale);
		fill = ((float)(p.res[POWER + i] / p.max[FOOD + i]) * 11 * scale);
		if (p.max[POWER + i] > 0) {
			sf::Color color = sf::Color(60, 143, 72);
			if (p.res[POWER + i] >= p.max[POWER + i]) {
				color = sf::Color(128, 33, 44);
			}
			drawSprite(16, 608, 1, 7, dX + 2 * scale, dY + 14 * scale, fill, scale);
		}
		dY += (sH + 1) * scale;
	}
	...
	for (int i = 0; i < 2; i++) {
		...
		Print(to_str(p.income[FOOD + i]), dX + 26.5 * scale, dY + 17.5 * scale, scale);
