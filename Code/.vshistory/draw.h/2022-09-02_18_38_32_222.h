...
void DrawMap() {
	...
	for (int i = ceil(y_max/2.0); i >= -y_max/2; i--) {
		for (int j = ceil(x_max/2.0); j >= -x_max/2; j--) {
			...
			if (mY >= yShift + i * 16 + texture.getSize().y / 2.0) {
				if (mY <= yShift + i * 16 + texture.getSize().y / 2.0 + 16) {
					if (mX >= xShift + j * 16 + texture.getSize().x / 2.0) {
						if (mY <= xShift + j * 16 + texture.getSize().x / 2.0 + 16) {
