...
void drawSprite(int sX, int sY, int w, int h, float dX, float dY, float scale = 1, sf::Color color = sf::Color(255, 255, 255)) {
...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int z) {
	float scale = z / 16.0;
	...
	//Draw the base tile
	drawSprite(sX + rX * 16, sY + 17 + rY * 16, 16, 16, dX, dY, scale);
	TERRAIN adjType;
	for (int a = -1; a < 2; a++) {
		for (int b = -1; b < 2; b++) {
			if ((a == 0) != (b == 0)) {
				...
				if (typePrecedent(adjType) > typePrecedent(map[y][x].type)) {
					...
					if (side > 0) {
						if (b == -1) {
							drawSprite(sX + 14, sY, 2*scale, 16*scale, dX - 8, dY, scale);
						}
						else {
							drawSprite(sX, sY, 2*scale, 16*scale, dX + 7, dY + 0, scale);
						}
					}
					else {
						if (a == -1) {
							drawSprite(sX + 16, sY + 14, 16*scale, 2*scale, dX, dY - 8, scale);
						}
						else {
							drawSprite(sX + 16, sY, 16*scale, 2*scale, dX, dY + 7, scale);
						}
					}
				}
			}
...
void DrawMap() {
	int x_max = ceil(W/zoom) + 3;
	int y_max = ceil(H/zoom) + 3;
	for (int i = -y_max/2.0; i < y_max/2.0; i++) {
		for (int j = -x_max/2.0; j < x_max/2.0; j++) {
			int y = safeC(i + yView);
			int x = safeC(j + xView);
			DrawTile(map, x, y, xShift + j * zoom + W/2.0, yShift + i * zoom + H/2.0, zoom);
		}
	}
}