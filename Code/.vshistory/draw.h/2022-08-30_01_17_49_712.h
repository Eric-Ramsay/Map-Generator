			...
			if ((a == 0) != (b == 0)) {
				...
				if (typePrecedent(adjType) > typePrecedent(map[y][x].type)) {
					...
					if (side > 0) {
						...
						if (b == -1) {
							drawSprite(sX + 14, sY, 2, 16, dX, dY, z);
						}
						else {
							drawSprite(sX, sY, 2, 16, dX + offset, dY, z);
						}
					}
					else {
						if ((offset - (int)offset) == .75) {
						...
						if (a == -1) {
							drawSprite(sX + 16, sY + 14, 16, 2, dX, dY, z);
						}
						else {
							drawSprite(sX + 16, sY, 16, 2, dX + offset, dY, z);
						}
					}
				}
			}
...
void DrawMap() {
	...
	for (int i = -1 - ceil(height / 2); i <= 1 + ceil(height / 2); i++) {
		for (int j = -1 - ceil(width / 2); j <= ceil(width / 2); j++) {
			int y = safeC(i + yView);
			int x = safeC(j + xView);
			DrawTile(map, x, y, (view.getSize().x / 2 + ((j * zoom) - dXView)), (view.getSize().y / 2 + ((i * zoom) - dYView)), zoom);
