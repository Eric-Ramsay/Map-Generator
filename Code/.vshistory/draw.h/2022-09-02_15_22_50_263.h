...
void DrawMap() {
	int x_max = ceil(texture.getSize().x * .8 / 16.0);
	int y_max = ceil(texture.getSize().y * .8 / 16.0);

	for (int i = -ceil(x_max/2.0); i <= y_max/2; i++) {
		for (int j = -ceil(y_max/2.0); j <= x_max/2; j++) {
			int y = safeC(i + yView);
			int x = safeC(j + xView);
