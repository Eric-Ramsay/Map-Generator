...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
	...
	if (zom >= 16) {
		...
		//drawSprite(int sX, int sY, int w, int h, float dX, float dY, int scale = 1, float rotation = 0.0f, sf::Color color = sf::Color(255, 255, 255))
		DrawSprite(sX + rX * 16, sY + 17 + rY * 16, dX, dY, flat, olc::vf2d(), olc::vf2d(16, 16), olc::vf2d(z, z));
