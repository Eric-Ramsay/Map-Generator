...
void DrawBuilding(BUILDING type, int x, int y, int dX, int dY, sf::Color color = sf::Color(255, 255, 255)) {
	int sY = 80;
	int sX = 16 * (type - 1);
	if (type == ROAD) {
		sY = 112;
...
void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY) {
	...
	if (zoom >= 1) {
		...
		if (map[y][x].elev == HILL || map[y][x].elev == MOUNTAIN || map[y][x].forest != NONE || map[y][x].building > -1) {
			useRand = 0;
		}
