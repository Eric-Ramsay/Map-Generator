				...
				if (typePrecedent(adjType) > typePrecedent(map[y][x].type)) {
					...
					if (side > 0) {
						...
						if (a == -1) {
							...
							//drawSprite(sX + 16, sY, 16, 2, dX, dY + 0);
...
void DrawMap() {
	int x_max = ceil(view.getSize().x/16.0) + 3;
	int y_max = ceil(view.getSize().y/16.0) + 3;
	for (int i = -y_max/2.0; i < y_max/2.0; i++) {
		for (int j = -x_max/2.0; j < x_max/2.0; j++) {
			int y = safeC(i + yView);
			int x = safeC(j + xView);
			DrawTile(map, x, y, xShift + j * 16 + view.getCenter().x, yShift + i * 16 + view.getCenter().y);
		}
	}
}