...
void genOceans(std::vector< std::vector<MapTile>>& map, int size, int num, int elev) {
	int s = size;
	int n = num;
	int dir = 0;
	int lastDir = 0;
	...
	while (s > 0 && n > 0) {
		if (s == 1)	{
			n--;
			s = oceanSize; //Ocean size is too small, go to new location to begin making ocean
		...
		else
		{
			...
			map[yCoord][xCoord].elevation = elev;
			if (elev <= 0) {
...
std::vector<std::vector<Tile>> createMap() {
	...
	genOceans(map, MODI * MAPSIZE / 10 * MAPSIZE / 10 * MAPSIZE / 10, 28 * MODI, -100);
