...
void rivers(std::vector< std::vector<MapTile >>& map) {
	...
	auto makeRiver = [&](int yC, int xC) {
		...
		int minDir = -1;
		int y = -1; int x = -1;
		int min = map[yC][xC].elevation - 1;
		float water = map[yC][xC].rain / 100.0;
		while (stack.size() > 0 && !stop && water > 0) {
			water -= .1;
			...
			if (map[y][x].elevation > 0) {
				map[y][x].river += water;
				map[y][x].elevation -= .01;
				min = map[yC][xC].elevation;
				minDir = -1;
				if (map[safeC(y - 1)][x].elevation <= min && lastDir != 0 && lastlast != 0 && bannedDir != 0) {
					min = map[safeC(y - 1)][x].elevation;
					minDir = 0;
				}
				if (map[y][safeC(1 + x)].elevation <= min && lastDir != 1 && lastlast != 1 && bannedDir != 1) {
					min = map[y][safeC(1 + x)].elevation;
					minDir = 1;
				}
				if (map[safeC(y + 1)][x].elevation <= min && lastDir != 2 && lastlast != 2 && bannedDir != 2) {
					min = map[safeC(y + 1)][x].elevation;

					minDir = 2;
				}
				if (map[y][safeC(x - 1)].elevation <= min && lastDir != 3 && lastlast != 3 && bannedDir != 3) {
			...
			else if (map[y][x].type == 'a' || map[y][x].type == 'w') {
				stop = true;
			}
			rS++;
		}
	...
	for (int i = 0; i < MAPSIZE; i++) {
		//std::cout << i << std::endl;
		for (int j = 0; j < MAPSIZE; j++) {
			if (map[i][j].river > 200) {
				map[i][j].type = 'r';
				map[i][j].elevation = -100;
				map[i][j].rain = 260;
				map[i][j].id[2] = map[i][j].id[0];
			}
		}
	}
...
std::vector<std::vector<Tile>> createMap(std::vector<std::vector<MapTile>>& base) {
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			map[i][j].rain += (50 - map[i][j].elevation / 2);
		}
	}
