							...
							for (int b = -1; b < 2; b++) {
								if (a * b == 0 && a != b) {
									...
									if (map[safeC(i + a)][safeC(j + b)].forest) {
										treeChance += 5;
										map[i][j].forestChance += 20 * min(1, map[i][j].rain / 1000.0);
									}
								}
							}
						}
						int chance = (treeChance + map[i][j].forestChance/4.0) * (min(1, map[i][j].rain / 1000.0) * (.5 + .5 * min(1, map[i][j].heat / 25.0)) * min(1, (100 - map[i][j].elevation)/10));
						if (rand() % 100 < chance) {
							map[i][j].forest = true;
						}
					}
...
std::vector<std::vector<Tile>> createMap(std::vector<std::vector<MapTile>>& base) {
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			map[i][j].forestChance -= map[i][j].elevation / 3;
