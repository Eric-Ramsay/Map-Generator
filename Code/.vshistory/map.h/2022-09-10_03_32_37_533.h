...
void rivers(std::vector< std::vector<MapTile >>& map) {
	...
	auto makeRiver = [&](int yC, int xC) {
		...
		while (open.size() > 0 && rS++ < MAX_SIZE && !stop) {
			...
			for (int a = -1; a < 2; a++) {
				for (int b = -1; b < 2; b++) {
					if (a != b && (a == 0 || b == 0)) {
						if (ap[safeC(current.y + a)][safeC(current.x + b)].elevation <= 0) {
					...
					for (int b = -1; b < 2; b++) {
						if (a != b && (a == 0 || b == 0)) {
							...
							if (!found) {
								...
								if (!found) {
									consider.h = map[safeC(current.y + a)][safeC(current.x + b)].elevation;
									consider.g = (consider.h + current.g)/10.0;
	...
	for (int i = 0; i < MAPSIZE; i += 1) {
		...
		for (int j = 0; j < MAPSIZE; j += 1) {
			if (map[i][j].elevation > 70 && map[i][j].elevation < 95) {
				int chance = 500;
				if ((rand() % 1000) > chance) {
					makeRiver(i, j);
					if (super_stop) {
