			...
			while (i < MAPSIZE && i >= 0) {
				...
				while (j < MAPSIZE && j >= 0) {
					...
					if (map[i][j].elevation > 0) {
						...
						if (avg < map[i][j].elevation) {
							map[i][j].elevation -= .2;
						}
						else if (avg > map[i][j].elevation) {
							map[i][j].elevation += .5;
						}
						//map[i][j].elevation = rand() % 3 + (Max * (.75 + ((((double)rand() / RAND_MAX)) * .2))) * (100 + 2 * MODI) / 100.0;
					}
...
std::vector<std::vector<Tile>> createMap() {
	...
	elev(map, 20, (int)round(40.0 * MODI), 160);
	elev(map, 10, (int)round(80.0 * MODI), 160);
	elev(map, (int)(40.0 * MODI), (int)round(20.0 * MODI), 150);
	elev(map, (int)(320.0 * MODI), (int)round(20.0 * MODI), 150);
	elev(map, (int)(400.0 * MODI), (int)round(35.0 * MODI), 150);
