...
void finalize(std::vector< std::vector<MapTile >>& map) {
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (t.elevation > 0) {
				//Cold Biomes
				float lushness = 2 * map[i][j].heat + map[i][j].rain;
				if (lushness < 255) {
					if (map[i][j].heat < 20) {
						map[i][j].type = 'c';
					}
				...
				else if (lushness < 300) {
					map[i][j].type = 'b';
				}
				else if (lushness < 350) {
					map[i][j].type = 'o';
				}
				else if (lushness < 395) {
					map[i][j].type = 'q';
				}
				else if (lushness < 420) {
					map[i][j].type = 'v';
				}
				else if (lushness < 450) {
					map[i][j].type = 'p';
				}
				else if (lushness < 500) {
					map[i][j].type = 'g';
				}
				else {
					map[i][j].type = 'm';
