			...
			if (t.elevation > 0) {
				...
				if (map[i][j].heat < 25) {
					if (map[i][j].rain < 225) {
						...
						map[i][j].type = '1' + (map[i][j].rain - 250) / 50;
					}
					else {
						map[i][j].type = 'm';
					}
					...
					if (map[i][j].rain < 200) {
						...
						map[i][j].type = '1' + (map[i][j].rain - 225) / 50;
					}
					else {
						map[i][j].type = 'm';
					}
					...
					if (map[i][j].rain < 175) {
						...
						map[i][j].type = '1' + (map[i][j].rain - 200) / 25;
					}
					else {
						map[i][j].type = 'm';
					}
			...
			while (i < MAPSIZE && i >= 0) {
				...
				while (j < MAPSIZE && j >= 0) {
					...
					if (map[i][j].elevation > 0) {
						...
						map[i][j].heat = (map[i][j].heat * 3 + avgH) / 4.0;
					}
					if (h == 0) { j++; }
					else { j--; }
				}
...
std::vector<std::vector<Tile>> createMap(std::vector<std::vector<MapTile>>& base) {
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (map[i][j].heat > map[i][j].elevation / 8) {
				map[i][j].heat -= (map[i][j].elevation / 8);
			}
		}
	}
