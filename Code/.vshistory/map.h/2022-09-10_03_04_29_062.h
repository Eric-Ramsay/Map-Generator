...
void rivers(std::vector< std::vector<MapTile >>& map) {
	auto makeRiver = [&](int yC, int xC) {
		...
		while (open.size() > 0 && rS++ < MAX_SIZE && !stop) {
			...
			if (map[current.y][current.x].elevation <= 0) {
				stop = true;
			}
			else {
				...
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
						if (a != b && (a == 0 || b == 0)) {
							found = false;
							Spot consider = Spot(safeC(current.x + a), safeC(current.y + b), closed.size());
							...
							if (!found) {
								...
								if (!found) {
									consider.g = map[safeC(current.x + a)][safeC(current.y + b)].elevation;
	...
	for (int i = 0; i < MAPSIZE; i += 1) {
		...
		for (int j = 0; j < MAPSIZE; j += 1) {
			if (map[i][j].elevation > 10 && map[i][j].elevation < 95) {
				int chance = 990;
				if ((rand() % 1000) > chance) {
					makeRiver(i, j);
				}
...
void lakes(std::vector< std::vector<MapTile >>& map) {
	auto makeLake = [&](int yC, int xC) {
		...
		bool found;
		bool stop = false;
		int elevation = map[yC][xC].elevation;
		while (open.size() > 0 && !stop) {
			min = 0;
			for (int i = 1; i < open.size(); i++) {
				if (open[i].g < open[min].g) {
					min = i;
		...
		if (!stop && rS <= MAX_SIZE && rS >= MIN_SIZE) {
			for (Spot c : closed) {
				map[c.y][c.x].type = 'a';
