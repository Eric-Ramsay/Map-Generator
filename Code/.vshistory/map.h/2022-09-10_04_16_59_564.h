...
void lakes(std::vector< std::vector<MapTile >>& map) {
	...
	auto makeLake = [&](int yC, int xC) {
		...
		bool found;
		bool stop = false;
		int elev = map[yC][xC].elevation * 1.08;
		while (open.size() > 0 && rS++ < MAX_SIZE && !stop) {
			min = 0;
			for (int i = 1; i < open.size(); i++) {
				if (open[i].g + open[i].h < open[min].g + open[min].h) {
				...
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
						if (a != b && (a == 0 || b == 0)) {
							...
							if (consider.h < 60 && consider.h <= elev) {
								for (Spot s : closed) {
									if (s == consider) {
										found = true;
										break;
		...
		if (open.size() == 0 && rS < MAX_SIZE && rS > 25 && !stop) {
			while (current.parent != -1) {
				map[current.y][current.x].type = 'a';
				map[current.y][current.x].elevation = -100;
				current = closed[current.parent];
			}
			map[current.y][current.x].elevation = -100;
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			if (map[i][j].elevation > 15 && map[i][j].elevation < 65) {
				int chance = 900;
				if ((rand() % 1000) > chance) {
					makeLake(i, j);
				}
