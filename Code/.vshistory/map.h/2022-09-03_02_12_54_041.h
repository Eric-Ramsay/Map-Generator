				...
				while (j < MAPSIZE && j >= 0) {
					...
					if (map[i][j].elevation > 0) {
						...
						if (Min < 20) {
							...
							if (avg >= 50) {
								map[i][j].elevation = (rand() % 3 + ((Max / 2 + avg / 2) * (.88))) * (100 + pow(MODI, 2)) / 100.0;
							}
							else {
...
std::vector<std::vector<Tile>> createMap() {
	...
	elev(map, 20, (int)round(40.0 * MODI), 120);
	elev(map, 10, (int)round(80.0 * MODI), 120);
	elev(map, (int)(40.0 * MODI), (int)round(20.0 * MODI), 120);
	elev(map, (int)(320.0 * MODI), (int)round(20.0 * MODI), 120);
	elev(map, (int)(400.0 * MODI), (int)round(35.0 * MODI), 120);
