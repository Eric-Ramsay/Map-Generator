					...
					if (map[i][j].elevation > 0) {
						...
						if (map[i][j].elevation < Max) {
							...
							if (map[i][j].elevation >= 85) {
								map[i][j].elevation = (map[i][j].elevation * 2 + avg) / 3.0;
							}
							else if (map[i][j].elevation >= 50) {
								map[i][j].elevation = (map[i][j].elevation * 9 + avg) / 10.0;
							}
							else {
								map[i][j].elevation = (map[i][j].elevation * 5 + avg) / 6.0;
							}
						}
						else {
							map[i][j].elevation = (map[i][j].elevation * 9 + avg) / 10.0;
...
std::vector<std::vector<Tile>> createMap(std::vector<std::vector<MapTile>>& base) {
	...
	elev(map, MODI * MAPSIZE / 10 * MAPSIZE / 10 * MAPSIZE / 10, 28 * MODI, 40);
	...
	elev(map, (int)(400.0 * MODI), (int)round(200.0 * MODI), 120);
	elev(map, (int)(1600.0 * MODI), (int)round(80.0 * MODI), 120);
	elev(map, (int)(2000.0 * MODI), (int)round(140.0 * MODI), 120);
