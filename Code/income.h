...
void BuildingIncome(int x, int y, float(&wIncome)[WATER + 1], float(&wMax)[WATER + 1], float (&sIncome)[WATER + 1], float (&sMax)[WATER + 1], float mult = 1.0) {
	...
	if (type == HOUSE) {
		...
		income[STONE] += .3 + map[y][x].elev/40.0;
		m[STONE] += 10;
	}
...
void Income(Player& p) {
			...
			if (i == 0) {
				...
				if (map[c.y][c.x].building == -1) {
					if (map[c.y][c.x].forest != BARE) {
						...
						case OCEAN: case SHALLOW: gIncome[FOOD] += 1; gIncome[GOLD] += .03; break;
	...
	if (sIncome[S_JOBS] > 0) {
		sEff = min(1, (int)p.res[SPECIALISTS] / (float)sIncome[S_JOBS]);
	}
	//Worker Efficiency
	if (wIncome[W_JOBS] > 0) {
		wEff = min(1, (int)p.res[WORKERS] / (float)wIncome[W_JOBS]);
	}
	if (gIncome[G_JOBS] > 0 && p.res[WORKERS] > wIncome[W_JOBS]) {
		gEff = min(1, (int)(p.res[WORKERS] - wIncome[W_JOBS]) / (float)gIncome[G_JOBS]);
	}
